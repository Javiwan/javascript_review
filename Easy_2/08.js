/*
Write a function that returns an Array that contains every other 
element of an Array that is passed in as an argument. The values 
in the returned list should be those values that are in the 1st, 
3rd, 5th, and so on elements of the argument Array.

input: array
output: array

- Iterate over the array
  - If array index % 2 diferent to 0 add that element to the final array
  - Return the final array

*/

const oddities = (arr) => arr.filter((_, idx) => idx % 2 === 0);
const eventies = (arr) => arr.filter((_, idx) => idx % 2 === 1);

console.log(oddities([2, 3, 4, 5, 6])); // logs [2, 4, 6]
console.log(eventies([2, 3, 4, 5, 6])); // logs [2, 4, 6]
console.log(oddities([1, 2, 3, 4, 5, 6])); // logs [1, 3, 5]
console.log(eventies([1, 2, 3, 4, 5, 6])); // logs [1, 3, 5]
console.log(oddities(["abc", "def"])); // logs ['abc']
console.log(eventies(["abc", "def"])); // logs ['abc']
console.log(oddities([123])); // logs [123]
console.log(eventies([123])); // logs [123]
console.log(oddities([])); // logs []
console.log(eventies([])); // logs []